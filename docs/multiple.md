...
both **Django Google SSO** and **Django Microsoft SSO** for Page login but only  **Django Microsoft SSO** for the Admin, you can add the respective
`*_SSO_PAGES_ENABLED` and `*_SSO_ADMIN_ENABLED`, like this:

```python
--8<-- "example_microsoft_app/settings.py:sso_split"
```
!!! warning "You need to be explicit on these settings"
    If you set `MICROSOFT_SSO_ADMIN_ENABLED = False` and do not set `MICROSOFT_SSO_PAGES_ENABLED`, the default value for `MICROSOFT_SSO_PAGES_ENABLED` is also `False`.
